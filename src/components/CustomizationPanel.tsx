...
export const CustomizationPanel: React.FC<Props> = ({ profile, onUpdateProfile }) => {
  ...
  const [storeCatalogItems, setStoreCatalogItems] = React.useState<any[]>([]);

  React.useEffect(() => {
    fetch(`${API_BASE_URL}/api/admin/store-items?t=${Date.now()}`)
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) setStoreCatalogItems(data);
      })
      .catch(err => console.error('Error loading store items in customization panel:', err));
  }, []);

  // Dynamically merged options from hardcoded presets + server store items
  const dynamicAvatars = React.useMemo(() => {
    const custom = storeCatalogItems
      .filter(i => i.category === 'avatar' && !AVATAR_OPTIONS.some(o => o.id === i.id))
      .map(i => ({ id: i.id, name: i.name, description: i.description || 'Özel Avatar' }));
    return [...AVATAR_OPTIONS, ...custom];
  }, [storeCatalogItems]);

  const dynamicThemes = React.useMemo(() => {
    const custom = storeCatalogItems
      .filter(i => i.category === 'board_theme' && !THEME_OPTIONS.some(o => o.id === i.id))
      .map(i => ({ id: i.id, name: i.name, color: i.previewColor || '#581C87', description: i.description || 'Özel Masa Teması' }));
    return [...THEME_OPTIONS, ...custom];
  }, [storeCatalogItems]);

  const dynamicCardBacks = React.useMemo(() => {
    const custom = storeCatalogItems
      .filter(i => i.category === 'card_back' && !CARD_BACKS.some(o => o.id === i.id))
      .map(i => ({ id: i.id, name: i.name, color: i.previewColor || '#EF4444', pattern: '✨' }));
    return [...CARD_BACKS, ...custom];
  }, [storeCatalogItems]);

  const dynamicFrames = React.useMemo(() => {
    const custom = storeCatalogItems
      .filter(i => i.category === 'profile_frame' && !PROFILE_FRAMES.some(o => o.id === i.id))
      .map(i => ({ id: i.id, name: i.name, description: i.description || 'Özel Profil Çerçevesi' }));
    return [...PROFILE_FRAMES, ...custom];
  }, [storeCatalogItems]);

  const dynamicPlayerBoards = React.useMemo(() => {
    const custom = storeCatalogItems
      .filter(i => i.category === 'player_board' && !PLAYER_BOARDS.some(o => o.id === i.id))
      .map(i => ({ id: i.id, name: i.name, description: i.description || 'Özel Oyuncu Tahtası' }));
    return [...PLAYER_BOARDS, ...custom];
  }, [storeCatalogItems]);

  const dynamicCardSkins = React.useMemo(() => {
    const custom = storeCatalogItems
      .filter(i => i.category === 'card_skin' && !CARD_SKINS.some(o => o.id === i.id))
      .map(i => ({ id: i.id, name: i.name, description: i.description || 'Özel Kart Skin' }));
    return [...CARD_SKINS, ...custom];
  }, [storeCatalogItems]);

  const dynamicActionVfx = React.useMemo(() => {
    const custom = storeCatalogItems
      .filter(i => i.category === 'action_vfx' && !ACTION_VFX.some(o => o.id === i.id))
      .map(i => ({ id: i.id, name: i.name, description: i.description || 'Özel Hamle Efekti' }));
    return [...ACTION_VFX, ...custom];
  }, [storeCatalogItems]);

  const dynamicSounds = React.useMemo(() => {
    const custom = storeCatalogItems
      .filter(i => i.category === 'celebration_sound' && !CELEBRATION_SOUNDS.some(o => o.id === i.id))
      .map(i => ({ id: i.id, name: i.name, description: i.description || 'Özel Zafer Sesi' }));
    return [...CELEBRATION_SOUNDS, ...custom];
  }, [storeCatalogItems]);
  ...
  const handleSaveSetting = async (key: keyof UserSettings, value: any) => {
    ...
    if (key === 'avatarId') {
      const storeItem = storeCatalogItems.find(i => i.id === value);
      updatedAvatarUrl = storeItem?.mediaUrl || (value.startsWith('http') || value.endsWith('.png') || value.endsWith('.jpg') || value.endsWith('.gif') || value.endsWith('.mp4') ? value : undefined);
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/settings/save`, {
        ...
        body: JSON.stringify({ userId: profile.id, settings: { [key]: value, avatarId: updatedProfile.avatarId } }),
      });
      if (response.ok) {
        ...
        if (data.avatarUrl || data.avatarId) {
          onUpdateProfile({
            ...updatedProfile,
            avatarId: data.avatarId || updatedProfile.avatarId,
            avatarUrl: data.avatarUrl || updatedProfile.avatarUrl,
          });
        }
