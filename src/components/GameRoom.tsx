...
export const GameRoom: React.FC<Props> = ({ roomId, isOffline, profile, onLeaveRoom, onUpdateProfile, adminSettings, roomPassword }) => {
  ...
  const [consecutivePlays, setConsecutivePlays] = React.useState(0);

  React.useEffect(() => {
    if (match) {
      setConsecutivePlays(0);
    }
  }, [match?.turnIndex]);
  ...
  const showcaseAnimationsEnabled = false;
  const setShowcaseAnimationsEnabled = (val: boolean) => {};
  ...
  const triggerScreenShake = (duration: number = 400) => {
    setScreenShake(true);
    setTimeout(() => {
  ...
  const [isCompactLayout, setIsCompactLayout] = React.useState(() => typeof window !== 'undefined' ? (window.innerWidth < 768 || window.innerHeight < 750) : true);

  const [glowingZones, setGlowingZones] = React.useState<Record<string, { colorHex: string; timestamp: number }>>({});

  const triggerZoneGlow = (playerId: string, zone: string, colorHex: string) => {
    const key = `${playerId}-${zone}`;
    setGlowingZones(prev => ({
      ...prev,
      [key]: { colorHex, timestamp: Date.now() }
    }));
    triggerHaptic('light');
    setTimeout(() => {
      setGlowingZones(prev => {
  ...
  React.useEffect(() => {
    ...
    const handleResize = () => {
      const compact = window.innerWidth < 768 || window.innerHeight < 750;
  ...
  const [storeCatalog, setStoreCatalog] = React.useState<any[]>([]);

  React.useEffect(() => {
    fetch(`${API_BASE_URL}/api/admin/store-items`)
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) setStoreCatalog(data);
      })
      .catch(err => console.error('Error fetching store items in GameRoom:', err));
  }, []);
  ...
  // Floating Animated Emotes & Quick Messages State
  const [floatingEmotes, setFloatingEmotes] = React.useState<{ id: string; emoji: string; text?: string; senderName: string; x: number; y: number }[]>([]);
  const [showEmoteMenu, setShowEmoteMenu] = React.useState(false);

  const triggerFloatingEmote = (emoji: string, text?: string) => {
    const id = `emote-${Date.now()}-${Math.random().toString(36).substr(2, 4)}`;
    const newEmote = {
      id,
      emoji,
      text,
    ...
    if (sounds.playCoin) sounds.playCoin(profile.settings);
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (distance > 15) {
      ...
      const padding = 55; // Forgiving touch zone padding
  ...
  const showActionToast = (
    ...
    // Disabled per user request to remove top action toasts
    return;
  };
  ...
  const prevMatchGlowRef = React.useRef<MatchState | null>(null);

  React.useEffect(() => {
    if (!match || !prevMatchGlowRef.current) {
      prevMatchGlowRef.current = match;
      return;
    }

    const prevMatch = prevMatchGlowRef.current;
    prevMatchGlowRef.current = match;

    match.players.forEach((player) => {
      const prevPlayer = prevMatch.players.find((p) => p.id === player.id);
      if (!prevPlayer) return;

      // 1. Compare property cards count
      const allColors: CardColor[] = ['brown', 'lightblue', 'pink', 'orange', 'red', 'yellow', 'green', 'darkblue', 'railroad', 'utility'];
      allColors.forEach((col) => {
        const currentCount = player.properties[col]?.cards.length || 0;
        const prevCount = prevPlayer.properties[col]?.cards.length || 0;
        if (currentCount > prevCount) {
          const hex = COLOR_HEX[col] || '#CBD5E1';
          triggerZoneGlow(player.id, col, hex);
        }
      });

      // 2. Compare bank count
      const currentBankCount = player.bank.length;
      const prevBankCount = prevPlayer.bank.length;
      if (currentBankCount > prevBankCount) {
        triggerZoneGlow(player.id, 'bank', '#10B981'); // Emerald green for bank vault
      }
    });
  }, [match]);
  ...
  const playPlaySound = (card?: Card) => {
    ...
    if (card && (card.actionType === 'house' || card.actionType === 'hotel')) {
      ...
      sounds.playPropertyPlace(profile.settings, consecutivePlays);
    }
  };
  const playCoinSound = () => {
    const now = Date.now();
  ...
  const playActionSound = (card?: Card) => {
    if (!card) {
      sounds.playActionCardPlay(profile.settings, consecutivePlays);
      return;
    }
        ...
        players: [
          {
            ...
            boardTheme: profile.settings?.boardTheme || 'theme_slate',
  ...
  const handleOfflineEndTurn = () => {
    ...
    match.logs.push({
      ...
      message: `${match.turnNumber}. Tur: Sıra ${nextPlayer.username} adlı yapay zekaya geçti.`,
      timestamp: Date.now(),
      turnNumber: match.turnNumber
    });
  ...
  const executeBotTurnOffline = (initialActionsCount = 0) => {
    ...
    const endBotTurn = () => {
      ...
      if (nextPlayer.isBot) {
        activeMatch.logs.push({
          id: `turn-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
          message: `${activeMatch.turnNumber}. Tur: Sıra ${nextPlayer.username} adlı yapay zekaya geçti.`,
          timestamp: Date.now(),
          turnNumber: activeMatch.turnNumber
        });
        ...
        activeMatch.logs.push({
          id: `turn-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
          message: `${activeMatch.turnNumber}. Tur: Sıra tekrar senin! Başarılar.`,
          timestamp: Date.now(),
          turnNumber: activeMatch.turnNumber
        });
        triggerOfflineDraw(activeMatch);
  ...
  const isRentCardPlayable = (card: Card): boolean => {
    if (card.type !== 'rent') return false;
    const ownedColors = Object.keys(localPlayer.properties).filter(col => {
    ...
    const color1 = card.color;
    const color2 = card.secondaryColor;
    return ownedColors.includes(color1) || !!(color2 && ownedColors.includes(color2));
  };
  ...
  return (
    ...
    >
      {/* 1. Dynamic Canvas Particle Background & Store Theme Renderer */}
      {(() => {
        const customTheme = storeCatalog.find(i => i.id === (profile.settings?.boardTheme || 'theme_slate'));
        if (customTheme?.mediaUrl) {
          const isVid = customTheme.mediaType === 'video' || customTheme.mediaUrl.endsWith('.mp4') || customTheme.mediaUrl.endsWith('.webm');
          return (
            <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
              {isVid ? (
                <video src={customTheme.mediaUrl} autoPlay loop muted className="w-full h-full object-cover opacity-60" />
              ) : (
                <img src={customTheme.mediaUrl} alt="Masa Teması" className="w-full h-full object-cover opacity-60" />
              )}
            </div>
          );
        }
        return <CanvasBackground theme={profile.settings?.boardTheme || 'theme_classic'} />;
      })()}

      {/* ✍️ Custom Table Engraving Overlay */}
      {(() => {
        const engravingText = profile.settings?.tableEngraving || profile.username;
        return (
          <div className="fixed inset-0 pointer-events-none z-0 flex items-center justify-center opacity-30 select-none">
            <div className="text-center font-black tracking-[0.3em] uppercase text-amber-400/50 text-2xl sm:text-4xl border-y-2 border-amber-400/25 py-2 px-8 rounded-full shadow-[0_0_50px_rgba(245,158,11,0.2)] drop-shadow-[0_0_15px_rgba(245,158,11,0.4)] backdrop-blur-[0.5px]">
              🎲 {engravingText} 🎲
            </div>
          </div>
        );
      ...
      {/* 💬 Live Floating Emotes & Quick Messages Overlay */}
        ...
        <AnimatePresence>
          {floatingEmotes.map(emote => (
            <motion.div
              ...
              className="absolute flex flex-col items-center gap-1.5 drop-shadow-[0_10px_25px_rgba(0,0,0,0.9)]"
            >
              <span className="text-6xl animate-bounce-subtle filter drop-shadow-[0_5px_10px_rgba(0,0,0,0.7)]">{emote.emoji}</span>
              {emote.text && (
                <span className="bg-slate-950/95 border-2 border-amber-400/80 text-amber-300 font-black text-xs px-3 py-1 rounded-full shadow-2xl tracking-wider uppercase">
                  {emote.text}
                </span>
              )}
              <span className="text-[9.5px] font-black text-slate-200 bg-black/75 px-2.5 py-0.5 rounded-full border border-white/10">
                {emote.senderName}
              </span>
            </motion.div>
          ))}
        ...
        <header className="border-b border-white/5 bg-black/45 backdrop-blur-md px-2 py-1.5 sm:px-4 sm:py-2 flex items-center justify-between z-40 flex-shrink-0 animate-fade-in gap-1 sm:gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 relative">
            ...
            <div id="game-toolbar" className="flex items-center gap-1 bg-slate-950/40 border border-white/[0.06] rounded-xl p-0.5 sm:p-1 shadow-lg backdrop-blur-sm">
              ...
              <button
                id="toolbar-compact-btn"
                onClick={() => {
                  playPlaySound();
                  setIsCompactLayout(!isCompactLayout);
                }}
                className={`p-1 px-1.5 sm:px-2 sm:py-1 rounded-lg text-[10px] font-bold transition-all flex items-center gap-1 cursor-pointer select-none ${isCompactLayout
                  ? 'bg-amber-500/25 border border-amber-500/40 text-amber-300 shadow-[0_0_8px_rgba(245,158,11,0.2)]'
                  : 'bg-white/[0.01] border border-white/5 hover:bg-white/[0.05] text-slate-400 hover:text-white'
                  }`}
                title={profile.settings.language === 'en' ? "Compact Layout" : "Sıkışık Düzen"}
              >
                <span>🗜️</span>
                <span className="hidden md:inline">{profile.settings.language === 'en' ? 'Compact' : 'Sıkışık'}</span>
              </button>

              {/* Floating Emote & Quick Message Button */}
              <button
                id="toolbar-emote-btn"
                onClick={() => {
                  playPlaySound();
                  setShowEmoteMenu(!showEmoteMenu);
                }}
                className={`p-1 px-1.5 sm:px-2.5 sm:py-1 rounded-lg text-[10px] font-bold transition-all flex items-center gap-1 cursor-pointer select-none ${showEmoteMenu
                  ? 'bg-amber-500/20 border border-amber-500/40 text-amber-300 shadow-[0_0_10px_rgba(245,158,11,0.15)]'
                  : 'bg-white/[0.01] border border-white/5 hover:bg-white/[0.05] text-slate-400 hover:text-white'
                  }`}
                title="Tepki Emojileri ve Hızlı Mesajlar"
              >
                <span>😁</span>
                <span className="hidden md:inline">Tepki</span>
              </button>
            ...
            {/* Floating Animated Emotes Popup Menu */}
            {showEmoteMenu && (
              <div className="absolute top-full right-16 mt-2 z-50 bg-slate-950/95 border border-amber-400/40 rounded-2xl p-3 shadow-2xl backdrop-blur-md w-64 space-y-3">
                <div className="flex justify-between items-center pb-2 border-b border-white/10">
                  <span className="text-[11px] font-black text-amber-300 uppercase tracking-wider">🎉 Tepki & Hızlı Mesajlar</span>
                  <button onClick={() => setShowEmoteMenu(false)} className="text-slate-400 hover:text-white text-xs">✕</button>
                </div>

                {/* Animated Emojis */}
                <div>
                  <span className="text-[9px] text-slate-400 font-bold block mb-1.5">REAKSİYON EMOJİLERİ</span>
                  <div className="grid grid-cols-5 gap-1.5">
                    {[
                      { emoji: '🐱😭', label: 'Ağlayan Kedi' },
                      { emoji: '🔥', label: 'Alev' },
                      { emoji: '🤣', label: 'Kahkaha' },
                      ...
                      <button
                        key={idx}
                        onClick={() => triggerFloatingEmote(item.emoji)}
                        className="p-1.5 bg-white/5 hover:bg-amber-500/20 border border-white/10 rounded-xl text-xl hover:scale-125 transition-all cursor-pointer flex items-center justify-center"
                        title={item.label}
                      >
                        {item.emoji}
                      </button>
                ...
                {/* Quick Messages */}
                <div>
                  <span className="text-[9px] text-slate-400 font-bold block mb-1.5">HIZLI MESAJLAR</span>
                  <div className="grid grid-cols-2 gap-1.5">
                    {[
                      { emoji: '👏', text: 'İyi Hamle!' },
                      { emoji: '⚡', text: 'Hızlı Oyna!' },
                      { emoji: '🙏', text: 'Teşekkürler!' },
                      { emoji: '🛡️', text: 'Savunma Var!' },
                    ].map((msg, idx) => (
                      <button
                        key={idx}
                        onClick={() => triggerFloatingEmote(msg.emoji, msg.text)}
                        className="px-2 py-1.5 bg-slate-900 hover:bg-amber-500/20 border border-white/10 text-left rounded-lg text-[10px] font-bold text-slate-200 hover:text-amber-300 transition-all cursor-pointer flex items-center gap-1.5"
                      >
                        <span>{msg.emoji}</span>
                      ...
                      <div key={p.id} className="flex items-center justify-between p-2.5 bg-black/40 border border-white/5 rounded-xl transition-all hover:bg-black/50">
                        <div className="flex items-center gap-2">
                          <AvatarWithFrame
                            ...
                            customFrameItem={storeCatalog.find(i => i.id === (p.profileFrame || 'frame_none'))}
          ...
          <div className="px-3 pt-2.5 flex-shrink-0">
            ...
            <div className={isOpponentsGrid
              ...
              {match.players.map((p) => {
                ...
                const customBoardItem = storeCatalog.find(i => i.id === p.playerBoard);
                const bStyle = PLAYER_BOARD_STYLES[p.playerBoard || 'board_classic'] || {
                  bgClass: customBoardItem?.mediaUrl ? 'bg-slate-900/90' : 'bg-slate-900/80',
                  borderClass: 'border-indigo-500/40',
                  glowClass: 'shadow-lg shadow-indigo-950/40',
                ...
                return (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 pointer-events-none opacity-40 mix-blend-color-dodge overflow-hidden">
                      {customBoardItem?.mediaUrl && (
                        (customBoardItem.mediaType === 'video' || customBoardItem.mediaUrl.endsWith('.mp4') || customBoardItem.mediaUrl.endsWith('.webm')) ? (
                          <video src={customBoardItem.mediaUrl} autoPlay loop muted className="w-full h-full object-cover" />
                        ) : (
                          <img src={customBoardItem.mediaUrl} alt="Board" className="w-full h-full object-cover" />
                        )
                      )}
                      {customBoardItem?.options?.gradientColors && (
                        <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${customBoardItem.options.gradientColors.join(', ')})` }} />
                      )}
                    ...
                    <div className="flex flex-col mt-2 pt-1.5 border-t border-white/[0.04] gap-1.5 relative z-10">
                      ...
                      <div className="flex flex-wrap gap-1 justify-start py-0.5">
                        {Object.keys(p.properties).map((colorKey) => {
                          ...
                          const isGlowing = glowingZones[`${p.id}-${col}`];
                          return (
                            <div
                              key={col}
                              className={`flex-shrink-0 w-[24px] h-[34px] bg-white rounded flex flex-col justify-between p-0.5 relative overflow-hidden shadow-sm border ${isGlowing ? 'zone-highlight-glow' : ''} ${isCompleted ? 'border-amber-400 ring-1 ring-amber-400/30 shadow-[0_0_4px_rgba(251,191,36,0.5)]' : 'border-slate-300'
                                }`}
                              style={isGlowing ? { '--glow-color': isGlowing.colorHex } as any : undefined}
          ...
          <div className={`flex-1 min-h-0 flex flex-col justify-around px-2 sm:px-3 transition-all ${isCompactLayout ? 'py-0.5 space-y-1' : 'py-1.5 space-y-2'
            ...
            <div className={`grid grid-cols-3 gap-1.5 sm:gap-2 items-center bg-black/15 border border-white/[0.04] rounded-xl transition-all ${isCompactLayout ? 'p-1.5' : 'p-2.5'
              ...
              <div className="text-center space-y-0.5">
                <span className="text-[7.5px] text-slate-400 uppercase tracking-widest block font-bold">DESTE / TUR</span>
                <div
                ...
                <div className="flex flex-col items-center gap-0.5 mt-1">
                  <span className="text-[7px] text-amber-300 font-black block bg-amber-500/10 px-1.5 py-0.5 rounded-full inline-block leading-none">
                    {match.deckCount} Kart
                  </span>
                  <span className="text-[6.5px] text-indigo-300 font-black block bg-indigo-500/15 border border-indigo-500/20 px-1.5 py-0.5 rounded-full inline-block leading-none">
                    {match.turnNumber || 1}. Tur
                  </span>
                </div>
              </div>
            ...
            <div className="flex gap-2 items-stretch flex-1 min-h-0">
              ...
              <div className={`bg-[#090C12]/40 border border-white/5 rounded-xl flex flex-col justify-between transition-all shrink-0 ${isCompactLayout ? 'p-1 sm:p-1.5 space-y-1 w-[68px] sm:w-[84px] md:w-[96px]' : 'p-2 space-y-1.5 w-[84px] sm:w-[104px] md:w-[118px]'
                ...
                {(() => {
                  ...
                  const isBankGlowing = glowingZones[`${localPlayer.id}-bank`];
                  ...
                  return (
                    <div
                      ...
                      className={`flex-1 flex flex-col items-center justify-center p-1 rounded-xl transition-all cursor-pointer ${isBankGlowing ? 'zone-highlight-glow' : ''} ${isDragOverBank
                      ...
                      style={isBankGlowing ? { '--glow-color': isBankGlowing.colorHex } as any : undefined}
                    >
              ...
              <div className={`bg-[#090C12]/40 border border-white/5 rounded-xl flex-1 flex flex-col justify-center min-h-0 transition-all ${isCompactLayout ? 'p-1 sm:p-1.5 space-y-1' : 'p-2 space-y-1.5'
                ...
                >
                  ...
                  {Object.keys(localPlayer.properties).map((colorKey) => {
                    ...
                    const isGlowing = glowingZones[`${localPlayer.id}-${col}`];
                    ...
                    return (
                      <div
                        ...
                        className={`flex flex-col items-center rounded-xl transition-all duration-300 w-[48px] sm:w-[60px] md:w-[72px] shrink-0 relative cursor-pointer select-none property-set-card-trigger ${isGlowing ? 'zone-highlight-glow' : ''} ${isExpanded
          ...
          <div className="bg-slate-900/90 border-t border-white/10 flex-shrink-0 relative z-30">
            ...
            {isHandExpanded && (
              <div className="p-2.5">
                ...
                <div className={`w-full max-w-full flex overflow-x-auto pb-1.5 justify-start scrollbar-thin transition-all ${isCompactLayout ? 'gap-1' : 'gap-2'}`}>
                  {(() => {
                    ...
                    return sortedHand.map((card, index) => {
                      ...
                      const baseRotate = (index - (totalCards - 1) / 2) * 4;
                      const baseY = Math.abs(index - (totalCards - 1) / 2) * 3;

                      let xOffset = 0;
                      let cardScale = 1;
                      let cardY = baseY;
                      let cardRotate = baseRotate;
                      let cardZIndex = 10 + index;

                      const hoveredHandCardIndex = hoveredCard ? sortedHand.findIndex((c) => c.id === hoveredCard.id) : -1;

                      if (hoveredHandCardIndex !== -1) {
                        if (index === hoveredHandCardIndex) {
                          cardScale = 1.18;
                          cardY = -30;
                          cardRotate = 0;
                          cardZIndex = 100;
                        } else if (index < hoveredHandCardIndex) {
                          xOffset = -25;
                          cardScale = 0.95;
                          cardZIndex = 10 + index;
                        } else if (index > hoveredHandCardIndex) {
                      ...
                      return (
                        <motion.div
                          ...
                          onMouseEnter={() => setHoveredCard(card)}
                          onMouseLeave={() => setHoveredCard(null)}
                          className={`cursor-grab active:cursor-grabbing select-none touch-pan-x flex-shrink-0 ${draggingCard?.id === card.id ? 'opacity-40 scale-95' : ''
                        ...
                        >
                          <GameCard
                            ...
                            activeEffect={cardEffects[card.id] || (isMyTurn && isRentCardPlayable(card) ? 'rent-ready' : null)}
                            disable3D={disable3D}
                  ...
                  const parseEnrichedLogs = (logs: GameLog[]) => {
                    ...
                    return logs.map((log) => {
                      ...
                      if (msg.includes('Sıra') || msg.includes('sırasını') || msg.includes('turunu') || msg.includes('başladı') || msg.includes('Oyun başladı')) {
                        if (log.turnNumber === undefined && (msg.includes('Sıra') || msg.includes('başladı'))) {
                          currentTurn++;
                          currentAction = 0;
                        }
                        return {
              ...
              <div className="grid grid-cols-2 gap-2.5 max-h-[280px] overflow-y-auto pr-1 scrollbar-thin">
                {(() => {
                  ...
                  possibleColors.sort((a, b) => {
                    const hasA = (localPlayer.properties[a]?.cards.length || 0) > 0 ? 1 : 0;
                    const hasB = (localPlayer.properties[b]?.cards.length || 0) > 0 ? 1 : 0;
                    return hasB - hasA;
                  });
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Bank status summary */}
                {(() => {
                  const isBankGlowing = glowingZones[`${opponent.id}-bank`];
                  return (
                    <div
                      className={`bg-slate-950/80 border rounded-2xl p-3 space-y-2 text-center relative select-none transition-all duration-300 ${isBankGlowing ? 'zone-highlight-glow' : 'border-slate-800'}`}
                      style={isBankGlowing ? { '--glow-color': isBankGlowing.colorHex } as any : undefined}
                    >
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {Object.keys(opponent.properties).map((colorKey) => {
                      ...
                      const rentVal = calculateSetRent(set.cards, col, set.hasHouse, set.hasHotel);
                      const colorHex = COLOR_HEX[col];
                      const isGlowing = glowingZones[`${opponent.id}-${col}`];

                      return (
                        <div
                          key={col}
                          className={`bg-slate-950/60 border rounded-2xl p-2.5 flex flex-col justify-between transition-all duration-300 relative ${isGlowing ? 'zone-highlight-glow' : ''} ${isCritical
      ...
      {/* 11. Central Action Card Showcase Overlay - Removed */}
