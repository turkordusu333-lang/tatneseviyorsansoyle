...
import { AvatarWithFrame, loadGlobalStoreCatalog } from './AvatarWithFrame';
...
export const AdminDashboard: React.FC<Props> = ({ onSettingsUpdated }) => {
  const [activeTab, setActiveTab] = useState<'analytics' | 'rules' | 'players' | 'quests' | 'achievements' | 'tournaments' | 'translations' | 'voices' | 'store'>('analytics');
  
  // Store Catalog CRUD state
  const [adminStoreItems, setAdminStoreItems] = useState<any[]>([]);
  const [storeCategoryFilter, setStoreCategoryFilter] = useState<string>('avatar');
  const [editingStoreItem, setEditingStoreItem] = useState<any | null>(null);
  const [isNewStoreItem, setIsNewStoreItem] = useState<boolean>(false);
  const [storeForm, setStoreForm] = useState<any>({
    ...
    options: {
      ...
      glowColor: '#ec4899',
      gradientColors: '#00ffcc, #ff007f, #7928ca',
      animation: 'none',
      glow: true,
      particles: true,
      bubbles: true,
  ...
  const fetchAdminStoreItems = () => {
    fetch(`${API_BASE_URL}/api/admin/store-items`)
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) setAdminStoreItems(data);
      })
      .catch(err => console.error('Error fetching admin store items:', err));
  };
  ...
  const handleOpenAddStoreItem = () => {
    ...
    setIsNewStoreItem(true);
    setEditingStoreItem(true);
  };

  const handleOpenEditStoreItem = (item: any) => {
    const hasBorder = !!(item.options?.borderWidth && Number(item.options.borderWidth) > 0);
    const hasGlow = !!(item.options?.glow || item.options?.glowColor);
    const gradArr = item.options?.gradientColors;
    const hasGradient = Array.isArray(gradArr) ? gradArr.length > 0 : typeof gradArr === 'string' && gradArr.trim().length > 0;

    setStoreForm({
      ...item,
      options: {
        useBorder: hasBorder,
        borderWidth: item.options?.borderWidth || 0,
        borderColor: item.options?.borderColor || '',
        useGlow: hasGlow,
        glowColor: item.options?.glowColor || '',
        useGradient: hasGradient,
        gradientColors: Array.isArray(gradArr) ? gradArr.join(', ') : (gradArr || ''),
        animation: item.options?.animation || 'none',
        glow: item.options?.glow ?? false,
        particles: item.options?.particles ?? false,
        bubbles: item.options?.bubbles ?? false,
        lights: item.options?.lights ?? false,
        shake: item.options?.shake ?? false,
      }
    });
    setIsNewStoreItem(false);
    setEditingStoreItem(true);
  };

  const handleSaveStoreItem = (e: React.FormEvent) => {
    e.preventDefault();
    const opts = storeForm.options || {};
    
    let finalGradient: string[] = [];
    if (opts.gradientColors) {
      if (typeof opts.gradientColors === 'string') {
        finalGradient = opts.gradientColors.split(',').map((s: string) => s.trim()).filter((s: string) => s.length > 0);
      } else if (Array.isArray(opts.gradientColors)) {
        finalGradient = opts.gradientColors;
      }
    }
    ...
    const payload = {
      ...
      previewColor: storeForm.previewColor || opts.previewColor || '#EF4444',
      options: {
        ...opts,
        borderWidth: Number(opts.borderWidth || 3),
        glow: opts.useGlow !== false && (opts.glow || !!opts.glowColor),
    ...
    const url = isNewStoreItem
      ? `${API_BASE_URL}/api/admin/store-items`
      : `${API_BASE_URL}/api/admin/store-items/${storeForm.id}`;
    const method = isNewStoreItem ? 'POST' : 'PUT';
    ...
    fetch(url, {
      ...
      .then(data => {
        if (data.success) {
          setAdminStoreItems(data.items);
          setEditingStoreItem(null);
          loadGlobalStoreCatalog(true);
          setNotification({ message: isNewStoreItem ? 'Yeni ürün eklendi!' : 'Ürün güncellendi!', type: 'success' });
        } else {
          setNotification({ message: 'İşlem başarısız.', type: 'error' });
        }
      })
      .catch(err => {
        console.error(err);
  ...
  const handleDeleteStoreItem = (itemId: string) => {
    if (!window.confirm('Bu mağaza ürününü silmek istediğinize emin misiniz?')) return;

    fetch(`${API_BASE_URL}/api/admin/store-items/${itemId}`, {
      method: 'DELETE'
    })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          setAdminStoreItems(data.items);
          loadGlobalStoreCatalog(true);
          setNotification({ message: 'Ürün silindi!', type: 'success' });
        }
      })
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-64 bg-slate-950/40 border-r border-slate-900 flex flex-col p-4 gap-2">
          ...
          <button
            ...
            className={`w-full px-4 py-3 rounded-xl text-left text-sm font-semibold flex items-center gap-3 transition-all ${
              activeTab === 'store' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-slate-400 hover:bg-slate-900/60 hover:text-slate-200'
            }`}
          >
            <span>🛍️</span> Mağaza & Ürün Kataloğu
          </button>
        ...
        <div className="flex-1 p-6 overflow-y-auto bg-slate-950/20">
          ...
          {/* TAB 9: STORE CATALOG MANAGEMENT */}
          {activeTab === 'store' && (
            <div className="space-y-6">
              <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl">
                <div className="flex flex-wrap items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-800">
                  <div>
                    <h3 className="text-xl font-extrabold text-amber-400 flex items-center gap-2">
                      <span>🛍️</span> Mağaza & Ürün Kataloğu Yönetimi
                    </h3>
                    <p className="text-xs text-slate-400 mt-1">
                      Mağazaya yeni Avatar, Kart Sırtı, Masa Teması, Çerçeve, Oyuncu Tahtası, Resim/GIF/Video ve Efekt ekleyin, düzenleyin ve silin.
                    </p>
                  </div>
                  <button
                    onClick={handleOpenAddStoreItem}
                    className="px-4 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-extrabold text-xs rounded-xl shadow-lg transition flex items-center gap-1.5 cursor-pointer"
                  >
                ...
                {/* Category Filters */}
                <div className="flex overflow-x-auto gap-2 mb-6 pb-2 border-b border-slate-800/80">
                  {[
                    { id: 'avatar', label: '👤 Avatarlar' },
                    { id: 'card_back', label: '🎴 Kart Sırtları' },
                    { id: 'board_theme', label: '🌌 Masa Temaları' },
                    { id: 'profile_frame', label: '🖼️ Çerçeveler' },
                    { id: 'player_board', label: '🛡️ Oyuncu Tahtaları' },
                    { id: 'card_skin', label: '💠 Kart Skinleri' },
                    { id: 'action_vfx', label: '🎆 Hamle VFX' },
                    { id: 'celebration_sound', label: '🔊 Sesler' },
                  ].map(cat => (
                    <button
                      key={cat.id}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {adminStoreItems
                    .filter(item => item.category === storeCategoryFilter)
                    .map(item => (
                      <div
                        key={item.id}
                        className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 flex flex-col justify-between hover:border-indigo-500/40 transition shadow-lg"
                      >
                        <div>
                          {/* Visual Preview Box */}
                          <div className="h-28 bg-slate-900 rounded-lg mb-3 flex items-center justify-center relative overflow-hidden p-2 border border-white/5">
                            {item.mediaUrl && (item.mediaType === 'video' || item.mediaUrl.endsWith('.mp4') || item.mediaUrl.endsWith('.webm')) ? (
                              <video src={item.mediaUrl} autoPlay loop muted className="w-full h-full object-cover rounded-md" />
                            ) : item.mediaUrl ? (
                              <img src={item.mediaUrl} alt={item.name} className="w-full h-full object-cover rounded-md" />
                            ) : (
                              <div className="text-center">
                                <span className="text-3xl">🎨</span>
                                <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-wider">{item.mediaType || 'Standart'}</p>
                              </div>
                            )}
                            <span className="absolute top-2 right-2 bg-amber-500/20 border border-amber-400/40 text-amber-300 text-[10px] font-black px-2 py-0.5 rounded-full shadow">
                              🪙 {item.price}
                            </span>
                          </div>
                        ...
                        <div className="flex gap-2 mt-4 pt-3 border-t border-slate-800/80">
                          <button
                            ...
                            className="flex-1 py-1.5 bg-indigo-600/80 hover:bg-indigo-600 text-white text-xs font-bold rounded-lg transition cursor-pointer"
                          >
                            ✏️ Düzenle
                          </button>
                          <button
                            onClick={() => handleDeleteStoreItem(item.id)}
                            className="py-1.5 px-3 bg-rose-600/80 hover:bg-rose-600 text-white text-xs font-bold rounded-lg transition cursor-pointer"
                          >
                            🗑️ Sil
                          </button>
                        </div>
                  ...
                  {adminStoreItems.filter(item => item.category === storeCategoryFilter).length === 0 && (
                    <div className="col-span-full py-12 text-center text-slate-500 text-sm">
                      Bu kategoride ürün bulunmuyor. Yeni bir ürün eklemek için "Yeni Ürün Ekle" butonuna basın.
                    </div>
                  )}
                </div>
              </div>
              ...
              {editingStoreItem && (
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
                  <form
                    onSubmit={handleSaveStoreItem}
                    className="bg-slate-900 border-2 border-indigo-500/40 rounded-2xl w-full max-w-2xl p-6 space-y-4 shadow-2xl text-slate-200 text-xs"
                  >
                    <div className="flex justify-between items-center pb-3 border-b border-slate-800">
                      <h4 className="text-base font-extrabold text-amber-400">
                        {isNewStoreItem ? '✨ Yeni Ürün Ekle' : '✏️ Ürün Düzenle'}
                      </h4>
                      <button
                        type="button"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[11px] font-bold text-slate-400 mb-1">Ürün Adı</label>
                        <input
                          type="text"
                          required
                          value={storeForm.name}
                          onChange={(e) => setStoreForm({ ...storeForm, name: e.target.value })}
                          className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2 text-white focus:outline-none focus:border-indigo-400"
                          placeholder="Örn: Altın Ejderha Çerçevesi"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        >
                          ...
                          <option value="card_back">🎴 Kart Sırtı</option>
                          <option value="board_theme">🌌 Masa Teması</option>
                          <option value="profile_frame">🖼️ Profil Çerçevesi</option>
                          <option value="player_board">🛡️ Oyuncu Tahtası</option>
                          <option value="card_skin">💠 Kart Skin</option>
                          <option value="action_vfx">🎆 Hamle VFX</option>
                          <option value="celebration_sound">🔊 Zafer Sesi</option>
                        </select>
                      </div>

                      <div>
                        <label className="block text-[11px] font-bold text-slate-400 mb-1">Medya / Görsel Türü</label>
                        <select
                          value={storeForm.mediaType}
                          onChange={(e) => setStoreForm({ ...storeForm, mediaType: e.target.value })}
                          className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2 text-white"
                        >
                          <option value="image">Resim (PNG/JPG)</option>
                          <option value="gif">GIF (Hareketli Resim)</option>
                          <option value="video">Video (MP4/WebM)</option>
                          <option value="gradient">Gradient / Renk</option>
                          <option value="effect">CSS / Canvas Efekti</option>
                        </select>
                      </div>

                      <div>
                        <label className="block text-[11px] font-bold text-slate-400 mb-1">Fiyat (🪙 Jeton)</label>
                        <input
                          type="number"
                          min="0"
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-400 mb-1">Medya URL'si (Resim, GIF veya Video Bağlantısı)</label>
                      <input
                        type="text"
                        value={storeForm.mediaUrl}
                        onChange={(e) => setStoreForm({ ...storeForm, mediaUrl: e.target.value })}
                        className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2 text-white"
                        placeholder="https://images.unsplash.com/... veya https://assets.mixkit.co/videos/..."
                      />
                    </div>

                    {/* Dynamic Category Specific Settings Section */}
                    <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-800 space-y-3">
                      <h5 className="font-extrabold text-amber-400 text-[11px] uppercase tracking-wider flex items-center justify-between">
                        <span>✨ {storeForm.category.toUpperCase()} ÖZEL SEÇENEKLERİ</span>
                        <span className="text-[9px] text-slate-500 font-mono">Dinamik Özelleştirme</span>
                      </h5>

                      {/* 🎴 KART SIRTI ÖZEL SEÇENEKLERİ */}
                      {storeForm.category === 'card_back' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div>
                            <label className="block text-[10px] text-slate-400 mb-1 font-bold">Kart Sembolü / İkonu</label>
                            <input
                              type="text"
                              value={storeForm.options?.symbol || storeForm.symbol || '◆'}
                              onChange={(e) => setStoreForm({ ...storeForm, symbol: e.target.value, options: { ...storeForm.options, symbol: e.target.value } })}
                              className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-white text-xs"
                              placeholder="Örn: ◆, ★, ♛, 🔥, ❄️, ⚡, 🌌, 🌈"
                            />
                          </div>
                          <div>
                            <label className="block text-[10px] text-slate-400 mb-1 font-bold">Özel Renk Gradyanı (Virgülle Ayrılmış)</label>
                            <input
                              type="text"
                              value={typeof storeForm.options?.gradientColors === 'string' ? storeForm.options.gradientColors : (storeForm.options?.gradientColors?.join?.(', ') || '')}
                              onChange={(e) => setStoreForm({ ...storeForm, options: { ...storeForm.options, gradientColors: e.target.value } })}
                              className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-white text-xs"
                              placeholder="Örn: rgb(6, 78, 59), rgb(2, 6, 23) veya #064e3b, #020617"
                            />
                          </div>
                          <div>
                            <label className="block text-[10px] text-slate-400 mb-1 font-bold">Ana Renk / Önizleme Rengi (Hex)</label>
                            <input
                              type="text"
                              value={storeForm.previewColor || storeForm.options?.previewColor || '#EF4444'}
                              onChange={(e) => setStoreForm({ ...storeForm, previewColor: e.target.value, options: { ...storeForm.options, previewColor: e.target.value } })}
                              className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-white text-xs"
                              placeholder="#EF4444"
                            />
                          </div>
                          <div className="flex items-center gap-4 pt-4">
                            <label className="flex items-center space-x-2 text-[11px] text-slate-300 font-bold">
                              <input
                                type="checkbox"
                                checked={storeForm.options?.particles !== false}
                                onChange={(e) => setStoreForm({ ...storeForm, options: { ...storeForm.options, particles: e.target.checked } })}
                              />
                              <span>✨ Yüzen Parçacık Efekti</span>
                            </label>
                          </div>
                        </div>
                      )}

                      {/* 🖼️ PROFİL ÇERÇEVESİ ÖZEL SEÇENEKLERİ */}
                      {storeForm.category === 'profile_frame' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div>
                            <label className="block text-[10px] text-slate-400 mb-1 font-bold">Animasyon Türü</label>
                            <select
                              value={storeForm.options?.animation || 'pulse'}
                              onChange={(e) => setStoreForm({ ...storeForm, options: { ...storeForm.options, animation: e.target.value } })}
                              className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-white text-xs"
                            >
                              <option value="none">Yok / Düz</option>
                              <option value="pulse">Yanıp Sönme (Pulse)</option>
                              <option value="rotate">Dönme (Rotate)</option>
                              <option value="wave">Dalgalanma (Wave)</option>
                              <option value="shake">Titreşim (Shake)</option>
                              <option value="lava_flow">Lav Akışı (Lava Flow)</option>
                              <option value="rainbow_hue">Gökkuşağı Döngüsü (Rainbow)</option>
                            </select>
                          </div>
                          <div>
                            <label className="block text-[10px] text-slate-400 mb-1 font-bold">Çerçeve Kalınlığı (px)</label>
                            <input
                              type="number"
                              min="1"
                              max="10"
                              value={storeForm.options?.borderWidth || 3}
                              onChange={(e) => setStoreForm({ ...storeForm, options: { ...storeForm.options, borderWidth: Number(e.target.value) } })}
                              className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-white text-xs"
                            />
                          </div>
                          <div>
                            <label className="block text-[10px] text-slate-400 mb-1 font-bold">Parlama (Glow) Rengi (Hex)</label>
                            <input
                              type="text"
                              value={storeForm.options?.glowColor || '#EC4899'}
                              onChange={(e) => setStoreForm({ ...storeForm, options: { ...storeForm.options, glowColor: e.target.value } })}
                              className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-white text-xs"
                              placeholder="#EC4899"
                            />
                          </div>
                          <div>
                            <label className="block text-[10px] text-slate-400 mb-1 font-bold">Çerçeve Gradyan Renkleri</label>
                            <input
                              type="text"
                              value={typeof storeForm.options?.gradientColors === 'string' ? storeForm.options.gradientColors : (storeForm.options?.gradientColors?.join?.(', ') || '')}
                              onChange={(e) => setStoreForm({ ...storeForm, options: { ...storeForm.options, gradientColors: e.target.value } })}
                              className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-white text-xs"
                              placeholder="#ec4899, #8b5cf6, #3b82f6"
                            />
                          </div>
                        </div>
                      )}

                      {/* 🛡️ OYUNCU TAHTASI ÖZEL SEÇENEKLERİ */}
                      {storeForm.category === 'player_board' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div>
                            <label className="block text-[10px] text-slate-400 mb-1 font-bold">Atmosferik Işık Efekti</label>
                            <select
                              value={storeForm.options?.ambientEffect || 'cyber-grid'}
                              onChange={(e) => setStoreForm({ ...storeForm, options: { ...storeForm.options, ambientEffect: e.target.value } })}
                              className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-white text-xs"
                            >
                              <option value="none">Yok / Düz</option>
                              <option value="cyber-grid">💠 Siber Grid Izgarası</option>
                              <option value="magma-pulse">🌋 Volkanik Magma Nabzı</option>
                              <option value="galaxy-spin">🌌 Galaksi Girdabı</option>
                              <option value="ice-crystal">❄️ Donmuş Buz Kristalleri</option>
                              <option value="void-aura">🔮 Mistik Void Aurası</option>
                            </select>
                          </div>
                          <div>
                            <label className="block text-[10px] text-slate-400 mb-1 font-bold">Tahta Gradyan Renkleri</label>
                            <input
                              type="text"
                              value={typeof storeForm.options?.gradientColors === 'string' ? storeForm.options.gradientColors : (storeForm.options?.gradientColors?.join?.(', ') || '')}
                              onChange={(e) => setStoreForm({ ...storeForm, options: { ...storeForm.options, gradientColors: e.target.value } })}
                              className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-white text-xs"
                              placeholder="#1e1b4b, #311042, #020617"
                            />
                          </div>
                        </div>
                      )}

                      {/* 🔊 ZAFER SESİ ÖZEL SEÇENEKLERİ */}
                      {storeForm.category === 'celebration_sound' && (
                        <div className="space-y-3">
                          <div>
                            <label className="block text-[10px] text-slate-400 mb-1 font-bold">Ses Dosyası (MP3 Media URL veya Cihazdan Yükle)</label>
                            <div className="flex gap-2">
                              <input
                                type="text"
                                value={storeForm.mediaUrl || ''}
                                onChange={(e) => setStoreForm({ ...storeForm, mediaUrl: e.target.value })}
                                className="flex-1 bg-slate-900 border border-slate-800 rounded p-1.5 text-white text-xs"
                                placeholder="https://assets.mixkit.co/.../sound.mp3 veya dosya seçin"
                              />
                              <label className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded text-xs font-bold cursor-pointer flex items-center gap-1">
                                📁 MP3 Seç
                                <input
                                  type="file"
                                  accept="audio/*"
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (file) {
                                      const reader = new FileReader();
                                      reader.onload = (evt) => {
                                        if (evt.target?.result) setStoreForm({ ...storeForm, mediaUrl: evt.target.result as string });
                                      };
                                      reader.readAsDataURL(file);
                                    }
                                  }}
                          ...
                          {storeForm.mediaUrl && (
                            <div className="flex items-center gap-3 bg-slate-900 p-2 rounded-lg border border-slate-800">
                              <button
                                type="button"
                                onClick={() => {
                                  const a = new Audio(storeForm.mediaUrl);
                                  a.play().catch(console.error);
                                }}
                                className="px-3 py-1 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded text-xs cursor-pointer flex items-center gap-1"
                              >
                                ▶️ Sesi Test Et
                              </button>
                              <span className="text-[10px] text-slate-400">Ses hazır ve dinlenebilir.</span>
                            </div>
                          )}
                        </div>
                      )}

                      {/* 👤 AVATAR ÖZEL SEÇENEKLERİ */}
                      {storeForm.category === 'avatar' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div>
                            <label className="block text-[10px] text-slate-400 mb-1 font-bold">Avatar Emojisi</label>
                            <input
                              type="text"
                              value={storeForm.options?.symbol || storeForm.symbol || '👑'}
                              onChange={(e) => setStoreForm({ ...storeForm, symbol: e.target.value, options: { ...storeForm.options, symbol: e.target.value } })}
                    ...
                    <div className="flex justify-between items-center pt-3 border-t border-slate-800">
                      <button
                        ...
                        className="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-extrabold rounded-lg shadow-lg cursor-pointer flex items-center gap-1.5"
                      >
                        <span>🧪 Canlı Masa & Kart Simülasyonu</span>
                      </button>
                      ...
                      <div className="flex space-x-3">
                        <button
                          type="button"
                          onClick={() => setEditingStoreItem(null)}
                          className="px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-white font-bold cursor-pointer"
                        >
                          İptal
                        </button>
                        <button
                          type="submit"
                          className="px-6 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-extrabold rounded-lg shadow-lg cursor-pointer"
                        >
                          💾 Kaydet
                        </button>
                      </div>
              ...
              {/* 🧪 Admin Live Sandbox Simulator Modal */}
              {showSandboxModal && (
                <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-lg flex items-center justify-center p-4 overflow-y-auto">
                  <div className="bg-slate-950 border-2 border-amber-400/50 rounded-3xl w-full max-w-4xl p-6 space-y-6 shadow-2xl text-slate-200 relative overflow-hidden">
                    <div className="flex justify-between items-center pb-3 border-b border-white/10">
                      <div>
                        <h3 className="text-lg font-black text-amber-400 flex items-center gap-2">
                          <span>🧪</span> Admin Canlı Oyun İçi Simülasyonu (Sandbox)
                        </h3>
                        <p className="text-xs text-slate-400">Ürünün gerçek maç esnasında nasıl görüneceğini anında test edin.</p>
                      </div>
                      <button
                        onClick={() => setShowSandboxModal(false)}
                        className="text-slate-400 hover:text-white text-xl font-bold px-3 py-1 bg-white/10 rounded-full"
                      >
                        ✕
                      </button>
                    </div>
                    ...
                    {/* Simulator Stage */}
                    <div className="w-full h-80 rounded-2xl border-2 border-white/20 relative overflow-hidden flex flex-col justify-between p-4 shadow-2xl bg-slate-900">
                      {/* 1. Table Theme Layer */}
                      {storeForm.mediaUrl && (storeForm.category === 'board_theme' || storeForm.mediaType === 'video' || storeForm.mediaType === 'image') ? (
                        (storeForm.mediaType === 'video' || storeForm.mediaUrl.endsWith('.mp4') || storeForm.mediaUrl.endsWith('.webm')) ? (
                          <video src={storeForm.mediaUrl} autoPlay loop muted className="absolute inset-0 w-full h-full object-cover opacity-70 z-0" />
                        ) : (
                          <img src={storeForm.mediaUrl} alt="Live Theme" className="absolute inset-0 w-full h-full object-cover opacity-70 z-0" />
                        )
                      ) : (
                        <div 
                          className="absolute inset-0 bg-slate-950" 
                          style={{
                            background: storeForm.options?.useGradient && storeForm.options?.gradientColors
                              ? `linear-gradient(135deg, ${typeof storeForm.options.gradientColors === 'string' ? storeForm.options.gradientColors : storeForm.options.gradientColors.join(', ')})`
                              : undefined
                          }}
                        />
                      )}

                      {/* 2. Custom Table Engraving Text */}
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 opacity-30 select-none">
                        <div className="text-center font-black tracking-[0.3em] uppercase text-amber-400 text-xl sm:text-3xl border-y-2 border-amber-400/30 py-1.5 px-6 rounded-full drop-shadow-[0_0_15px_rgba(245,158,11,0.5)]">
                          🎲 {storeForm.name || 'DEAL MASTER PRO'} 🎲
                        </div>
                      </div>

                      {/* 3. Header Player Card Simulation */}
                      <div className="relative z-20 flex justify-between items-center w-full bg-slate-950/80 border border-white/10 p-3 rounded-2xl backdrop-blur-md">
                        <div className="flex items-center gap-3">
                          <AvatarWithFrame
                            avatarId={storeForm.category === 'avatar' ? storeForm.id : 'avatar_classic'}
                            avatarUrl={storeForm.category === 'avatar' ? (storeForm.mediaUrl || '') : ''}
                            frameId={storeForm.category === 'profile_frame' ? storeForm.id : 'frame_neon'}
                            customFrameItem={storeForm.category === 'profile_frame' ? storeForm : null}
                            sizeClassName="w-12 h-12 text-xl"
                            prestigeBadge="vip"
                          />
                          <div>
                            <span className="font-extrabold text-sm text-white block">Test Oyuncu (VIP)</span>
                            <span className="text-[10px] text-amber-400 font-mono">10,000,000M 🏦</span>
                          </div>
                        </div>
                      ...
                      {/* 4. Cards & Action Simulation Stage */}
                      <div className="relative z-20 flex justify-around items-center w-full px-4">
                        {/* Sample Card Back */}
                        <div className="w-20 h-28 rounded-xl border-2 border-white/30 relative overflow-hidden shadow-2xl bg-gradient-to-br from-red-900 to-slate-950 flex flex-col justify-between p-1.5">
                          {storeForm.category === 'card_back' && storeForm.mediaUrl ? (
                            (storeForm.mediaType === 'video' || storeForm.mediaUrl.endsWith('.mp4')) ? (
                              <video src={storeForm.mediaUrl} autoPlay loop muted className="absolute inset-0 w-full h-full object-cover" />
                            ) : (
                              <img src={storeForm.mediaUrl} alt="Card Back" className="absolute inset-0 w-full h-full object-cover" />
                            )
                          ) : null}
                          <div className="text-[6px] font-black text-white/80 select-none z-10">MONOPOLY</div>
                          <div className="self-center bg-slate-950/85 border border-amber-400/60 px-1.5 py-0.5 rounded-full shadow-lg text-[7px] font-black text-amber-300 z-10">
                            DEAL PRO
                          </div>
                          <div className="text-[6px] font-black text-white/80 text-right select-none z-10">EDITION</div>
                        </div>

                        {/* Sample Front Card with Card Skin */}
                        <div className="w-20 h-28 rounded-xl border-2 border-amber-400/80 bg-white text-slate-900 flex flex-col justify-between p-1.5 shadow-2xl relative overflow-hidden">
                          {storeForm.category === 'card_skin' && storeForm.mediaUrl && (
                            <div className="absolute inset-0 opacity-60 pointer-events-none mix-blend-screen overflow-hidden">
                              <img src={storeForm.mediaUrl} alt="Skin" className="w-full h-full object-cover" />
                            </div>
                          )}
                          <div className="bg-red-600 text-white font-black text-[7px] p-0.5 text-center rounded uppercase">KİRA KARTI</div>
                          <div className="text-center font-black text-xs text-slate-950 my-auto">💰 5M</div>
                          <div className="text-[6px] text-slate-500 font-bold text-center">HAMLE KARTI</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end pt-2 border-t border-white/10">
                      <button
                        onClick={() => setShowSandboxModal(false)}
                        className="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold rounded-xl shadow-lg cursor-pointer"
                      >
                        Tamamdır (Kapat)
                      </button>
                    </div>
