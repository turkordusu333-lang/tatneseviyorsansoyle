...
import { CARD_BACK_STYLES } from './ShopDialog';

let cachedStoreCatalog: any[] = [];
let fetchPromise: Promise<any> | null = null;

const loadGlobalStoreCatalog = (forceRefresh: boolean = false) => {
  ...
  fetchPromise = fetch(`${API_BASE_URL}/api/admin/store-items?t=${Date.now()}`)
    .then(res => res.json())
    .then(data => {
      if (Array.isArray(data)) cachedStoreCatalog = data;
...
export const GameCard: React.FC<GameCardProps> = ({
  ...
  const [storeCatalog, setStoreCatalog] = React.useState<any[]>(cachedStoreCatalog);

  React.useEffect(() => {
    loadGlobalStoreCatalog().then(items => {
      if (items && Array.isArray(items) && items.length > 0) setStoreCatalog(items);
    });
  }, []);
  ...
  const customCardBackItem = storeCatalog.find(i => i.id === cardBack);
  const customCardSkinItem = storeCatalog.find(i => i.id === cardSkin);
  ...
  const skinOverlayElement = !isFaceDown && (
    <>
      {customCardSkinItem?.mediaUrl && (
        <div className={`absolute inset-0 pointer-events-none z-35 opacity-50 overflow-hidden mix-blend-screen ${getSkinOverlayClass()}`}>
          {customCardSkinItem.mediaType === 'video' || customCardSkinItem.mediaUrl.endsWith('.mp4') || customCardSkinItem.mediaUrl.endsWith('.webm') ? (
            <video src={customCardSkinItem.mediaUrl} autoPlay loop muted className="w-full h-full object-cover" />
          ) : (
            <img src={customCardSkinItem.mediaUrl} alt="Skin" className="w-full h-full object-cover" />
          )}
        </div>
      )}
  ...
  const renderCardBack = () => {
    const presetStyle = CARD_BACK_STYLES[cardBack];
    const isVid = customCardBackItem?.mediaUrl && (customCardBackItem.mediaType === 'video' || customCardBackItem.mediaUrl.endsWith('.mp4') || customCardBackItem.mediaUrl.endsWith('.webm'));
    
    // Gradient parsing helper
    let cardBgStyle: string | undefined = undefined;
    if (customCardBackItem?.options?.gradientColors) {
      const gc = customCardBackItem.options.gradientColors;
      if (Array.isArray(gc) && gc.length > 0) {
        cardBgStyle = `linear-gradient(135deg, ${gc.join(', ')})`;
      } else if (typeof gc === 'string' && gc.trim().length > 0) {
        cardBgStyle = `linear-gradient(135deg, ${gc.trim()})`;
      }
    }
    if (!cardBgStyle && customCardBackItem?.previewColor) {
      cardBgStyle = customCardBackItem.previewColor;
    }
    if (!cardBgStyle && presetStyle && !presetStyle.bgClass) {
      cardBgStyle = presetStyle.color;
    }

    const hasParticles = customCardBackItem?.options?.particles !== false && (customCardBackItem?.options?.particles || customCardBackItem?.options?.useParticles || presetStyle?.pColor);
    const particleColor = customCardBackItem?.options?.particleColor || customCardBackItem?.options?.glowColor || presetStyle?.pColor || '#F59E0B';
    const cardSymbol = customCardBackItem?.options?.symbol || customCardBackItem?.symbol || presetStyle?.symbol || '◆';

    return (
      <div 
        id={`card-back-${card.id}`}
        onClick={onClick}
        className={`w-full h-full rounded-2xl overflow-hidden cursor-pointer shadow-2xl border-2 border-white/20 select-none ${presetStyle?.bgClass || ''} ${className}`}
        style={{
          background: cardBgStyle
        }}
      >
        {/* Floating Particles Overlay */}
        {hasParticles && !customCardBackItem?.mediaUrl && (
        ...
        {isVid ? (
          <video src={customCardBackItem.mediaUrl} autoPlay loop muted className="w-full h-full object-cover rounded-2xl" />
        ) : customCardBackItem?.mediaUrl ? (
          <img src={customCardBackItem.mediaUrl} alt="Kart Sırtı" className="w-full h-full object-cover rounded-2xl" />
        ) : (
          <div 
            className={`w-full h-full flex flex-col justify-between p-2 text-white relative ${presetStyle?.bgClass || (cardBgStyle ? '' : 'bg-gradient-to-br from-red-900 via-rose-950 to-black')}`}
            style={{ background: cardBgStyle }}
          >
            <span className="text-[7px] font-black tracking-wider opacity-60">DEAL</span>
            <span className="text-2xl self-center font-black animate-pulse drop-shadow-md">{cardSymbol}</span>
            <span className="text-[7px] font-black tracking-wider opacity-60 text-right">PRO</span>
          </div>
        )}

        {/* Embossed Monopoly Deal PRO Logo Overlay Template for custom media */}
        {customCardBackItem?.mediaUrl && (
          <div className="absolute inset-0 flex flex-col justify-between p-2 pointer-events-none z-10 bg-black/25">
            <div className="text-[6px] font-black text-white/80 select-none text-left tracking-widest drop-shadow-[0_1px_2px_rgba(0,0,0,0.9)]">MONOPOLY</div>
            <div className="self-center bg-slate-950/85 border border-amber-400/60 px-2 py-0.5 rounded-full shadow-xl text-[8.5px] font-black text-amber-300 tracking-wider flex items-center gap-1 backdrop-blur-sm scale-90">
              <span>🎲</span>
              <span>DEAL PRO</span>
            </div>
            <div className="text-[6px] font-black text-white/80 select-none text-right tracking-widest drop-shadow-[0_1px_2px_rgba(0,0,0,0.9)]">EDITION</div>
  ...
  // If simple rendering (disable3D)
  if (disable3D && isFaceDown) {
    return (
      <div 
        className={`relative inline-block rounded-2xl overflow-hidden ${
          size === 'normal' ? 'w-[114px] h-[170px]' : size === 'medium' ? 'w-[46px] h-[68px]' : 'w-[32px] h-[46px]'
        } ${className}`}
      >
        {renderCardBack()}
      </div>
  ...
  if (size === 'medium') {
      ...
      if (details.isRent) {
        ...
        return (
          <div
            id={`card-med-${card.id}`}
            onClick={onClick}
            className={`w-full aspect-[2/3] rounded-md bg-white border border-slate-350 shadow-md flex flex-col justify-between p-0.5 cursor-pointer hover:-translate-y-1 transition-all relative select-none ${holoClass} ${activeEffect === 'rent-ready' ? 'border-yellow-400 shadow-[0_0_12px_rgba(234,179,8,0.7)] animate-pulse' : ''} ${className}`}
          >
            <div
              className="absolute inset-px rounded flex flex-col justify-between p-0.5 border border-black/10"
              style={{
    ...
    return (
      <Holo
        rarity={rarity}
        className={`relative inline-block w-[46px] sm:w-[68px] md:w-[80px] flex-shrink-0 rounded-md overflow-hidden ${activeEffect === 'rent-ready' ? 'border border-yellow-400 shadow-[0_0_12px_rgba(234,179,8,0.7)]' : ''}`}
        style={tiltStyle}
      >
        {tooltipElement}
        {renderMediumBody()}
        {activeEffect === 'rent-ready' && (
          <div className="absolute top-0.5 left-1/2 -translate-x-1/2 bg-yellow-500 text-slate-950 text-[5px] font-black px-1 py-0.2 rounded-full shadow border border-yellow-350 z-40 animate-bounce flex items-center gap-0.2 whitespace-nowrap leading-none">
            <span>⚡</span>
            <span>HAZIR</span>
          </div>
        )}
        {activeEffect === 'rent-ready' && (
          <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden rounded-md">
            <div className="absolute -inset-5 bg-[radial-gradient(circle,rgba(234,179,8,0.35)_0%,transparent_60%)] animate-pulse" />
          </div>
        )}
        {isHovered && (
          <div 
            className="pointer-events-none absolute inset-0 z-35 mix-blend-overlay transition-opacity duration-150 rounded-md"
            style={{
              background: `linear-gradient(115deg, transparent 30%, rgba(255,255,255,0.4) ${hoverCoords.x}%, transparent 70%)`
            }}
          />
        )}
        {skinOverlayElement}
  ...
  return (
    <Holo
      rarity={rarity}
      className={`relative inline-block rounded-2xl overflow-hidden ${activeEffect === 'rent-ready' ? 'border border-yellow-400 shadow-[0_0_15px_rgba(234,179,8,0.75)]' : ''}`}
      style={tiltStyle}
    >
      {tooltipElement}
      <div
        ...
        className={`flex-shrink-0 w-[114px] h-[170px] rounded-2xl border-[3.5px] shadow-2xl transition-all relative select-none flex flex-col justify-between cursor-pointer overflow-hidden p-0.5 ${skin.borderClass} ${skin.fontClass} ${holoClass} ${
          isSelected
            ...
            : 'hover:-translate-y-2'
        } ${activeEffect === 'rent-ready' ? 'border-yellow-400 shadow-[0_0_20px_rgba(234,179,8,0.85)] animate-pulse' : ''} ${className}`}
      >
        {activeEffect === 'rent-ready' && (
          <div className="absolute top-1 left-1/2 -translate-x-1/2 bg-yellow-500 text-slate-950 text-[7.5px] font-black px-2 py-0.5 rounded-full shadow-lg border border-yellow-300 z-40 animate-bounce flex items-center gap-0.5 whitespace-nowrap">
            <span>⚡</span>
            <span>KİRA HAZIR</span>
          </div>
        )}
        {activeEffect === 'rent-ready' && (
          <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden rounded-2xl">
            <div className="absolute -inset-10 bg-[radial-gradient(circle,rgba(234,179,8,0.35)_0%,transparent_60%)] animate-pulse" />
          </div>
        )}
      ...
      {isHovered && (
        <div 
          className="pointer-events-none absolute inset-0 z-35 mix-blend-overlay transition-opacity duration-150 rounded-2xl"
          style={{
            background: `linear-gradient(115deg, transparent 30%, rgba(255,255,255,0.4) ${hoverCoords.x}%, transparent 70%)`
          }}
        />
  ...
  return (
    <div 
      ...
      style={{
        perspective: '1000px',
        width: size === 'normal' ? '114px' : size === 'medium' ? '46px' : '32px',
        height: size === 'normal' ? '170px' : size === 'medium' ? '68px' : '46px',
    ...
    >
      ...
      >
        ...
        <div 
          ...
          style={{ backfaceVisibility: 'hidden', transform: 'rotateY(0deg)', zIndex: isFaceDown ? 1 : 2 }}
        >
          {renderCardFront()}
        </div>
        ...
        <div 
          className="absolute inset-0 w-full h-full"
          style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)', zIndex: isFaceDown ? 2 : 1 }}
        >
          {renderCardBack()}
        </div>
      </motion.div>
