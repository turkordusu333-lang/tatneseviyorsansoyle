...
export const ShopDialog: React.FC<Props> = ({ profile, onUpdateProfile }) => {
  ...
  const [catalogItems, setCatalogItems] = React.useState<any[]>(STORE_ITEMS);

  React.useEffect(() => {
    fetch(`${API_BASE_URL}/api/admin/store-items?t=${Date.now()}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleBuy = async (itemId: string, price: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const storeItem = catalogItems.find(item => item.id === itemId);
        if (storeItem) {
          ...
          if (storeItem.category === 'avatar') {
            newAvatarId = storeItem.id;
            newAvatarUrl = storeItem.mediaUrl;
            newSettings.avatarId = storeItem.id;
          } else if (storeItem.category === 'profile_frame') {
            newSettings.profileFrame = storeItem.id;
          } else if (storeItem.category === 'board_theme') {
            newSettings.boardTheme = storeItem.id;
          } else if (storeItem.category === 'player_board') {
            newSettings.playerBoard = storeItem.id;
          } else if (storeItem.category === 'card_back') {
            newSettings.cardBack = storeItem.id;
          }

          const autoEquippedProfile = {
            ...updatedProfile,
            avatarId: newAvatarId,
            avatarUrl: newAvatarUrl,
            settings: newSettings
          };
          onUpdateProfile(autoEquippedProfile);

          // Save on server
          fetch(`${API_BASE_URL}/api/settings/save`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userId: profile.id, settings: newSettings, avatarId: newAvatarId })
                  ...
                  >
                    ...
                    {item.category === 'avatar' && (
                      <div className="w-18 h-18 rounded-full border-4 border-red-500/80 flex items-center justify-center text-4xl bg-gradient-to-br from-slate-800 to-slate-900 shadow-xl shadow-red-950/40 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 relative overflow-hidden">
                        {item.mediaUrl ? (
                          (item.mediaType === 'video' || item.mediaUrl.endsWith('.mp4') || item.mediaUrl.endsWith('.webm')) ? (
                            <video src={item.mediaUrl} autoPlay loop muted className="w-full h-full object-cover rounded-full" />
                          ) : (
                            <img src={item.mediaUrl} alt={item.name} className="w-full h-full object-cover rounded-full" />
                          )
                        ) : (
                          <span>{AVATAR_EMOJIS[item.id] || '👑'}</span>
                        )}
                    ...
                    {item.category === 'card_back' && (() => {
                      ...
                      const isVid = item.mediaUrl && (item.mediaType === 'video' || item.mediaUrl.endsWith('.mp4') || item.mediaUrl.endsWith('.webm'));
                      return (
                        <div className="relative p-1">
                          <div
                            ...
                            style={{
                              background: item.options?.gradientColors?.length
                                ? `linear-gradient(135deg, ${item.options.gradientColors.join(', ')})`
                                : (item.previewColor || (cBack.bgClass ? undefined : cBack.color)),
                              borderColor: item.options?.borderColor || cBack.pColor || 'rgba(255, 255, 255, 0.2)',
                              boxShadow: `0 8px 16px -4px rgba(0,0,0,0.5), 0 0 12px ${(item.options?.glowColor || cBack.pColor || '#FFFFFF')}25`
                            }}
                          >
                            {isVid ? (
                              <video src={item.mediaUrl} autoPlay loop muted className="absolute inset-0 w-full h-full object-cover" />
                            ) : item.mediaUrl ? (
                              <img src={item.mediaUrl} alt={item.name} className="absolute inset-0 w-full h-full object-cover" />
                            ) : (
                              <>
                                <div className="text-[6px] font-black text-white/30 select-none text-left tracking-wide z-10">DEAL</div>
                                <span className="text-white text-3xl font-black self-center drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)] select-none animate-pulse z-10">
                                  {cBack.symbol || '◆'}
                                </span>
                                <div className="text-[6px] font-black text-white/30 select-none text-right tracking-wide z-10">PRO</div>
                              </>
                            )}
                            <div className="absolute inset-1 border border-white/5 rounded-lg pointer-events-none z-20" />
                          </div>
                        </div>
                      );
                    })()}
                    ...
                    {item.category === 'board_theme' && (() => {
                      const tStyle = BOARD_THEME_STYLES[item.id] || { bgClass: 'bg-slate-900', icon: '🌌', badge: item.name, glowColor: 'rgba(255,255,255,0.1)' };
                      const isVid = item.mediaUrl && (item.mediaType === 'video' || item.mediaUrl.endsWith('.mp4') || item.mediaUrl.endsWith('.webm'));
                      return (
                        <div className="w-full h-full flex flex-col items-center justify-center transition-all duration-300 relative overflow-hidden">
                          {isVid ? (
                            <video src={item.mediaUrl} autoPlay loop muted className="absolute inset-0 w-full h-full object-cover opacity-80" />
                          ) : item.mediaUrl ? (
                            <img src={item.mediaUrl} alt={item.name} className="absolute inset-0 w-full h-full object-cover opacity-80" />
                          ) : (
                            <div className={`absolute inset-0 ${tStyle.bgClass}`} style={{ background: item.options?.gradientColors?.length ? `linear-gradient(135deg, ${item.options.gradientColors.join(', ')})` : undefined }} />
                          )}
                          <div className="text-3xl filter drop-shadow-[0_0_8px_rgba(255,255,255,0.15)] animate-bounce-subtle select-none z-10">
                            {tStyle.icon || '🌌'}
                          </div>
                          <div className="absolute bottom-2 bg-black/70 border border-white/10 px-2.5 py-0.5 rounded-full text-[8px] font-black tracking-wider uppercase text-amber-300 z-10">
                            {item.name}
                          </div>
                        </div>
                      );
                    ...
                    {item.category === 'player_board' && (() => {
                      ...
                      return (
                        <div
                          className="w-full h-20 rounded-xl border flex flex-col justify-between p-2 shadow-xl relative overflow-hidden group-hover:scale-105 transition-all"
                          style={{
                            background: item.options?.gradientColors?.length
                              ? `linear-gradient(135deg, ${item.options.gradientColors.join(', ')})`
                              : item.previewColor || 'linear-gradient(135deg, #1e293b, #0f172a)',
                            borderColor: item.options?.borderColor || 'rgba(255,255,255,0.2)'
                          }}
                        >
                          {isVid ? (
                            <video src={item.mediaUrl} autoPlay loop muted className="absolute inset-0 w-full h-full object-cover opacity-70" />
                          ) : item.mediaUrl ? (
                            <img src={item.mediaUrl} alt={item.name} className="absolute inset-0 w-full h-full object-cover opacity-70" />
                          ) : null}
                          <div className="flex justify-between items-center z-10">
                            <span className="text-xs font-black text-amber-300">{item.name}</span>
                            <span className="text-[10px] bg-black/60 px-1.5 py-0.5 rounded text-white font-mono">TAHTA</span>
                          </div>
                          <span className="text-[9px] text-slate-300 font-bold z-10">Oyuncu Alanı Stili</span>
                        </div>
                      );
                    })()}
              ...
              <div className="bg-black/40 border border-white/5 rounded-2xl aspect-[1.5] flex items-center justify-center relative overflow-hidden p-6 mb-6">
                {previewItem.category === 'avatar' && (
                  <div className="flex flex-col items-center gap-3 relative">
                    <div className="w-24 h-24 rounded-full border-4 border-red-500 flex items-center justify-center text-5xl bg-slate-800 shadow-xl animate-bounce-subtle z-10 relative overflow-hidden">
                      {previewItem.mediaUrl ? (
                        previewItem.mediaType === 'video' || previewItem.mediaUrl.endsWith('.mp4') || previewItem.mediaUrl.endsWith('.webm') ? (
                          <video src={previewItem.mediaUrl} autoPlay loop muted className="w-full h-full object-cover" />
                        ) : (
                          <img src={previewItem.mediaUrl} alt={previewItem.name} className="w-full h-full object-cover" />
                        )
                      ) : (
                        <span>{AVATAR_EMOJIS[previewItem.id] || '👑'}</span>
                      )}
                    </div>
                ...
                {previewItem.category === 'card_back' && (() => {
                  ...
                  const isVid = previewItem.mediaUrl && (previewItem.mediaType === 'video' || previewItem.mediaUrl.endsWith('.mp4') || previewItem.mediaUrl.endsWith('.webm'));
                  
                  let bgStyle: string | undefined = undefined;
                  if (previewItem.options?.gradientColors) {
                    const gc = previewItem.options.gradientColors;
                    if (Array.isArray(gc) && gc.length > 0) bgStyle = `linear-gradient(135deg, ${gc.join(', ')})`;
                    else if (typeof gc === 'string' && gc.trim().length > 0) bgStyle = `linear-gradient(135deg, ${gc.trim()})`;
                  }
                  if (!bgStyle && previewItem.previewColor) bgStyle = previewItem.previewColor;
                  if (!bgStyle && !cBack.bgClass) bgStyle = cBack.color;

                  const cardSymbol = previewItem.options?.symbol || previewItem.symbol || cBack.symbol || '◆';
                  ...
                  return (
                    <div className="flex flex-col items-center">
                      ...
                      >
                        {isVid ? (
                          <video src={previewItem.mediaUrl} autoPlay loop muted className="absolute inset-0 w-full h-full object-cover" />
                        ) : previewItem.mediaUrl ? (
                          <img src={previewItem.mediaUrl} alt={previewItem.name} className="absolute inset-0 w-full h-full object-cover" />
                        ) : (
                          <>
                ...
                {previewItem.category === 'board_theme' && (() => {
                  ...
                  return (
                    <div
                      className="w-full h-full rounded-xl p-4 flex flex-col justify-between transition-all duration-500 border border-white/10 relative overflow-hidden bg-slate-950"
                    >
                      {isVid ? (
                        <video src={previewItem.mediaUrl} autoPlay loop muted className="absolute inset-0 w-full h-full object-cover opacity-80 z-0" />
                      ) : previewItem.mediaUrl ? (
                        <img src={previewItem.mediaUrl} alt={previewItem.name} className="absolute inset-0 w-full h-full object-cover opacity-80 z-0" />
                      ) : (
                        <div className={`absolute inset-0 ${tStyle.bgClass}`} style={{ background: previewItem.options?.gradientColors?.length ? `linear-gradient(135deg, ${previewItem.options.gradientColors.join(', ')})` : previewItem.previewColor || undefined }} />
                      )}

                      <div className="w-full flex justify-between items-center border-b border-white/10 pb-2 relative z-10">
                        ...
                        <span className="text-[9px] font-mono text-white/60">
                          {previewItem.mediaUrl ? 'ÖZEL MEDYA MASA TEMASI' : 'CANLI MASA TEMASI'}
                        </span>
                      </div>

                      <div className="grid grid-cols-3 gap-3 my-auto relative z-10">
                        <div className="bg-black/60 border border-white/10 rounded-xl p-3 text-center text-[10px] font-bold shadow-md text-slate-300">Banka 🏦</div>
                        <div className="bg-amber-500/30 border border-amber-500/50 rounded-xl p-3 text-center text-[10px] font-bold text-amber-300 animate-pulse shadow-[0_0_15px_rgba(245,158,11,0.35)]">Deste 🃏</div>
                        <div className="bg-black/60 border border-white/10 rounded-xl p-3 text-center text-[10px] font-bold shadow-md text-slate-300">Mülkler 🏡</div>
                      </div>

                      <div className="text-center text-[10px] text-amber-300 font-black uppercase tracking-wider animate-pulse relative z-10 flex items-center justify-center gap-1.5">
                        <span>🌌</span>
                ...
                {previewItem.category === 'card_skin' && (
                  <div className="flex flex-col items-center">
                    <div className="w-28 h-40 rounded-xl border-2 border-white/10 bg-slate-800 flex flex-col justify-between p-3 shadow-2xl relative overflow-hidden">
                      ...
                      {previewItem.mediaUrl && (
                        previewItem.mediaType === 'video' || previewItem.mediaUrl.endsWith('.mp4') || previewItem.mediaUrl.endsWith('.webm') ? (
                          <video src={previewItem.mediaUrl} autoPlay loop muted className="absolute inset-0 w-full h-full object-cover opacity-75" />
                        ) : (
                          <img src={previewItem.mediaUrl} alt={previewItem.name} className="absolute inset-0 w-full h-full object-cover opacity-75" />
                        )
                      )}
                ...
                {previewItem.category === 'player_board' && (() => {
                  const bStyle = PLAYER_BOARD_STYLES[previewItem.id] || {
                    bgClass: 'bg-slate-900/90',
                    borderClass: 'border-slate-800',
                    textClass: 'text-slate-200',
                    glowClass: '',
                    nameTr: previewItem.name || 'Oyuncu Tahtası',
                    nameEn: previewItem.name || 'Player Board',
                  };
                  ...
                  const ambEffect = previewItem.options?.ambientEffect;

                  return (
                    <div className="flex flex-col items-center gap-3 w-full">
                      <div
                        className={`w-[260px] p-5 rounded-2xl border-3 shadow-2xl transition-all duration-500 relative overflow-hidden flex flex-col justify-between ${bStyle.bgClass} ${bStyle.borderClass} ${bStyle.glowClass}`}
                      ...
                      >
                        ...
                        <div className="absolute inset-0 pointer-events-none opacity-40 mix-blend-color-dodge">
                          {(previewItem.id === 'board_cyber' || ambEffect === 'cyber-grid') && (
                          ...
                          {(previewItem.id === 'board_void' || ambEffect === 'void-aura') && (
                      ...
                      <span className="text-[10px] text-amber-400 font-bold mt-2 animate-pulse uppercase tracking-wider">
                        ✨ {profile.settings.language === 'en' ? (bStyle.nameEn || previewItem.name) : (bStyle.nameTr || previewItem.name)} ÖNİZLEMESİ AKTİF ✨
                      </span>
                    </div>
                  );
                })()}
